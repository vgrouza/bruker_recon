...
def apply_phase_correction(image_space_in: np.ndarray, readout_axis: int):
    """
    ...
    if readout_axis == 1:
        image_space_in = np.transpose(image_space_in, (1, 0, 2, 3))
    elif readout_axis == 2:
        image_space_in = np.transpose(image_space_in, (2, 1, 0, 3))
    ...
    print('Optimal slope derived from CC was alpha = ' + str(alpha[np.argmax(rho_list)]))

    # return the phase-corrected image space and optimal slope
    return image_space_in, alpha[np.argmax(rho_list)]


def apply_misregistration_correction(image_space_in: np.ndarray, readout_axis: int):
    ...
    if readout_axis == 1:
        image_space_corr = np.transpose(image_space_corr, (1, 0, 2, 3))
    elif readout_axis == 2:
        image_space_corr = np.transpose(image_space_corr, (2, 1, 0, 3))
    ...
    return image_space_corr, beta_list[np.argmax(mi_list)]
